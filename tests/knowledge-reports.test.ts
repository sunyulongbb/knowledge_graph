...
function fixture() {
  ...
  db.run('CREATE TABLE nodes(id TEXT PRIMARY KEY, project_id INTEGER, name TEXT, aliases TEXT, tags TEXT, description TEXT, wiki_md TEXT, link TEXT)');
  ...
  db.run("INSERT INTO nodes VALUES ('Q1', 1, '美国对华贸易政策', '[]', '[\"贸易\",\"关税\"]', '介绍关税与供应链政策。', '', 'https://example.test/q1')");
  db.run("INSERT INTO nodes VALUES ('Q2', 1, '半导体出口管制', '[]', '[\"芯片\"]', '先进芯片出口管制措施。', '', '')");
  db.run("INSERT INTO nodes VALUES ('Q3', 1, '政策制定机构', '[]', '[]', '负责相关政策协调。', '', '')");
  db.run("INSERT INTO nodes VALUES ('Q4', 2, '其他应用贸易资料', '[]', '[\"贸易\"]', '不应跨应用出现。', '', '')");
