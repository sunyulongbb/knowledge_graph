(function () {
  ...
  let canCreate = false, editingProject = null, deletingProject = null, currentDetail = null;
  ...
  function render() {
    ...
    byId('applicationCards').innerHTML = items.map((project) => `<article class="application-card application-market-card">
      ...
      <div class="application-card-actions"><button class="btn primary" type="button" data-app-detail="${project.id}">查看详情</button>${!project.member ? `<button class="btn" type="button" data-app-maintenance="${project.id}">${window.authUser ? '申请维护' : '登录后申请'}</button>` : ''}<button class="btn" type="button" data-app-open="${escape(project.slug)}">打开应用</button></div></div></article>`).join('') || '<div class="application-empty"><i class="fa-regular fa-folder-open"></i><p>暂无应用。你可以创建一个应用，或到市场申请维护。</p></div>';
    byId('btnApplicationCreate').disabled = !canCreate;
    byId('btnApplicationCreate').title = canCreate ? '创建应用' : window.authUser ? '请联系管理员授予创建应用权限' : '请先登录';
  }
  ...
  async function loadDetails(id) {
    ...
    detailVersion++;
    byId('applicationAccess').hidden = true;
    const catalog = byId('applicationCatalog'), target = byId('applicationDetailContent');
    catalog.hidden = true; byId('applicationDetail').hidden = false; target.innerHTML = '<div class="application-detail-loading"><span class="loading-spinner"></span>正在加载应用详情…</div>';
    try {
      ...
      const maintenanceLabel = !window.authUser ? '登录后申请维护' : project.member ? '维护管理' : '申请维护';
      target.innerHTML = `<div class="application-detail-topbar"><button class="btn" type="button" data-app-detail-back><i class="fa-solid fa-arrow-left"></i> 返回应用</button><div><button class="btn" type="button" data-app-open="${escape(project.slug)}">打开应用</button><button class="btn ${!project.member && window.authUser ? 'primary' : ''}" type="button" data-app-access="${project.id}"><i class="fa-solid fa-user-gear" aria-hidden="true"></i> ${maintenanceLabel}</button>${project.editSettings ? '<button class="btn primary" type="button" data-app-edit><i class="fa-solid fa-pen" aria-hidden="true"></i> 编辑应用</button>' : ''}${project.deleteApplication ? '<button class="btn danger" type="button" data-app-delete><i class="fa-regular fa-trash-can" aria-hidden="true"></i> 删除</button>' : ''}</div></div>
  ...
  async function loadApplications() {
    ...
    try {
      ...
      projects = data.projects; canCreate = data.canCreate === true; render(); status('');
    } catch (error) { if (token === listVersion) status(error.message); }
  }
  async function loadAccess(id) {
    if (String(id) !== activeProject) return;
    const token = ++detailVersion, target = byId('applicationAccess');
    target.hidden = false; target.textContent = '正在加载应用权限…';
    try {
      ...
      target.innerHTML = `<div class="application-detail-section-head"><h3><i class="fa-solid fa-user-gear" aria-hidden="true"></i> 维护管理</h3><span>${project.owner ? '创建者' : project.member ? '维护成员' : '访客'}</span></div>
        <p class="muted">维护成员可在此应用中新增自己的知识。编辑应用、删除应用、管理成员和审批申请还需具备相应角色权限；成员授权由创建者管理。</p>
    ...
    } catch (error) { if (token === detailVersion) target.innerHTML = `<p role="alert">${escape(error.message)}</p><button type="button" class="btn" data-access-retry>重新加载维护管理</button>`; }
  }
  async function refreshAfterChange(id) {
    await Promise.all([loadApplications(), ...(activeProject === String(id) ? [loadDetails(id)] : []), loadNotifications(), window.loadProjectsToSidebar?.()]);
  }
  ...
  function openEditor(project = null) {
    if (project ? !project.editSettings : !canCreate) return;
    editingProject = project;
    const form = byId('applicationCreateForm'); form.reset();
    for (const key of ['title', 'description', 'image', 'link']) form.elements[key].value = project?.[key] || '';
    form.elements.name.value = project?.slug || '';
    form.elements.name.disabled = !!project;
    byId('applicationEditorTitle').textContent = project ? '编辑应用' : '创建应用';
    byId('btnApplicationSave').textContent = project ? '保存修改' : '创建应用';
    byId('applicationEditorStatus').textContent = project ? '应用短名用于访问地址，创建后不可修改。' : '创建后，你将成为应用创建者。';
    byId('applicationEditorDialog').showModal(); form.elements.title.focus();
  }
  byId('btnApplicationCreate').addEventListener('click', () => openEditor());
  byId('btnApplicationCreateCancel').addEventListener('click', () => byId('applicationEditorDialog').close());
  byId('applicationCreateForm').addEventListener('submit', async (event) => {
    ...
    try {
      ...
      if (project) Object.assign(payload, { name: project.slug, tags: project.tags, theme_color: project.theme_color });
      const data = await api(project ? '/api/kb/update_project' : '/api/applications', payload);
      byId('applicationEditorDialog').close();
      activeProject = String(data.project.id); await refreshAfterChange(data.project.id);
      const output = byId('applicationAccess').querySelector('[data-access-status]');
      if (output) output.textContent = project ? '应用已更新' : '应用创建成功';
    } catch (error) { byId('applicationEditorStatus').textContent = error.message; } finally { button.disabled = false; }
  });
  byId('btnApplicationDeleteCancel').addEventListener('click', () => byId('applicationDeleteDialog').close());
  byId('applicationDeleteForm').addEventListener('submit', async (event) => {
    event.preventDefault();
    const project = deletingProject, form = event.target, button = form.querySelector('[type=submit]');
    if (!project?.deleteApplication) return;
    if (form.elements.confirmName.value.trim() !== project.slug) { byId('applicationDeleteStatus').textContent = '应用短名不匹配，请重新输入'; return; }
    button.disabled = true;
    try {
      await api('/api/kb/delete_project', { name: project.slug, confirmName: form.elements.confirmName.value.trim() });
      byId('applicationDeleteDialog').close();
      infoVersion++; detailVersion++; activeProject = null; currentDetail = null;
      byId('applicationDetail').hidden = true; byId('applicationCatalog').hidden = false;
      await Promise.all([loadApplications(), window.loadProjectsToSidebar?.(), loadNotifications()]);
      status('应用已删除');
      if (new URLSearchParams(location.search).get('db') === project.slug) {
        const url = new URL(location.href); url.searchParams.delete('db'); url.searchParams.delete('node'); url.hash = 'view=applications'; location.href = url;
      }
    } catch (error) { byId('applicationDeleteStatus').textContent = error.message; } finally { button.disabled = false; }
  });
  byId('applicationsPanel').addEventListener('click', async (event) => {
    const button = event.target.closest('button'); if (!button) return;
    if (button.hasAttribute('data-app-edit')) { openEditor(currentDetail); return; }
    if (button.hasAttribute('data-app-delete')) {
      if (!currentDetail?.deleteApplication) return;
      deletingProject = currentDetail; byId('applicationDeleteForm').reset();
      byId('applicationDeleteName').textContent = deletingProject.slug; byId('applicationDeleteStatus').textContent = '';
      byId('applicationDeleteDialog').showModal(); return;
    }
    ...
    if (button.dataset.appMaintenance) { await loadDetails(button.dataset.appMaintenance); byId('applicationAccess').scrollIntoView({ behavior: 'smooth', block: 'start' }); return; }
    ...
    } catch (error) { const output = byId('applicationAccess').querySelector('[data-access-status]'); if (activeProject === id && output) output.textContent = error.message; } finally { button.disabled = false; }
  });
  ...
  notifications.addEventListener('click', async (event) => {
    ...
    try { await api('/api/notifications/read', { id: Number(item.dataset.notification) }); await loadNotifications(); if (item.dataset.project) { closeNotifications(); window.setViewMode('applications'); await loadDetails(item.dataset.project); if (activeProject === item.dataset.project) byId('applicationAccess').scrollIntoView({ behavior: 'smooth', block: 'start' }); } }
  ...
  window.addEventListener('kb-auth-change', () => { projects = []; canCreate = false; render(); byId('applicationEditorDialog').close(); byId('applicationDeleteDialog').close(); editingProject = null; deletingProject = null; loadNotifications(); window.loadProjectsToSidebar?.(); if (window.kbViewMode === 'applications') loadApplications(); if (activeProject) loadDetails(activeProject); });
  setInterval(() => { if (!document.hidden && window.authUser) loadNotifications(); }, 30000);
  window.loadApplications = loadApplications;
  window.openApplicationDetails = (id) => { window.setViewMode('applications'); return loadDetails(id); };
  loadNotifications();
