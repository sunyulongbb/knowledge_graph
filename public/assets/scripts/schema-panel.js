(function () {
  ...
  async function syncFTypeInputValue(value) {
    if (!fTypeInput) return;
    const normalized = (value || "").toString().trim();
    try {
      await populateFTypeDatalist();
    } catch {}
    ensureFTypeOption(normalized);
  }
  ...
  if (attrPropPickerList) {
    ...
    attrPropPickerList.addEventListener("change", () => {
      ...
      try {
        const propEl = document.getElementById("attrProp");
        const propLabelEl = document.getElementById("attrPropLabel");
        if (propEl) propEl.value = propId;
        if (propLabelEl) propLabelEl.value = label;
        if (attrPropSearchInput) attrPropSearchInput.value = label;
      } catch {}
      try {
        const el = document.getElementById("attrCurrentProp");
  ...
  window.syncFTypeInputValue = syncFTypeInputValue;
  ...
  function getAllClassTags() {
    ...
    normalizeTagArray(window.kbProjectNodeTags || []).forEach((tag) => {
      const key = tag.toLowerCase();
      if (!tagMap.has(key)) tagMap.set(key, tag);
    });
  ...
  function syncProjectTagsFromTableNodes() {
    const nodes = Array.isArray(window.kbTableNodes) ? window.kbTableNodes : [];
    const collected = [];
    nodes.forEach((node) => {
      if (Array.isArray(node?.tags)) {
        collected.push(...node.tags);
      } else if (node?.tags) {
        collected.push(...String(node.tags).split(/[\n,，;；、]+/));
      }
    });
  ...
  function registerNodeTagsToTagManager(tags) {
    window.kbProjectNodeTags = normalizeTagArray([
      ...(window.kbProjectNodeTags || []),
  ...
  window.registerNodeTagsToTagManager = registerNodeTagsToTagManager;
})();
